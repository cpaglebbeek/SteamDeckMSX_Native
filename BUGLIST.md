...
### BUG-025 (geel, OPEN) — elke build hercompileert openMSX: cache-miss is structureel, niet door wijzigingen — gemeten 2026-07-26
...
- **Openstaand spoor:** `skip: derived` (70 MB build-output in de bronmap) staat in het manifest maar de notitie "hielp NIET" stond al in de projectmemory. Volgende stap is meten of de miss verdwijnt als `derived/` er domweg niet staat — dat is één opruimactie plus twee builds, en sluit het spoor definitief af.
- **Architectonisch:** een `type: dir`-source waarvan de map ook build-output bevat, vermengt bron en resultaat. Zolang dat zo is, is cache-gedrag niet redeneerbaar maar alleen meetbaar.
