...
### BUG-024 (geel, OPEN) — openMSX mag nergens schrijven: waarschuwing over het spel heen, save-states vermoedelijk stuk — gevonden 2026-07-26
- **Functioneel:** zichtbaar op het verificatie-screenshot van v0.3.3 (`docs/verification/v0.3.3/02-spel.png`): over het Nemesis-titelscherm staat een openMSX-dialoog "Couldn't save SRAM cbios-msx2.cmos (Error creating dir /root/.openMSX/persistent/C-BIOS_MSX2)". Elke speelsessie begint dus met een foutmelding, en spellen die hun voortgang in SRAM bewaren verliezen die.
- **Technisch:** `--filesystem=home:ro` (BUG-017-fix, sinds v0.3.1) mount de echte home read-only ín de sandbox, en HOME wijst daar naartoe — dus `~/.openMSX/` is onschrijfbaar. Dat raakt SRAM, `settings.xml` én naar alle waarschijnlijkheid de save-states achter de X-toets; dat laatste is nog niet apart getoetst.
- **Voorgestelde fix:** `--persist=.openMSX` als finish-arg. Flatpak legt die map dan in `~/.var/app/nl.icthorse.SteamDeckMSX/` — schrijfbaar, zonder de read-only home op te geven. Verifiëren mét de save-state-route: X-toets, state wegschrijven, app herstarten, state terugladen.
- **Architectonisch:** de BUG-017-fix koos read-only toegang tot de héle home om de scanner te bedienen. Die keuze is voor lezen juist, maar de emulator heeft óók een eigen schrijfplek nodig; dat is bij het verruimen van de leesrechten over het hoofd gezien. Lees- en schrijfpaden horen apart belegd te worden.
